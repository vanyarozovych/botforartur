...
async def on_member_remove(member):
    ...
    try:
        with sqlite3.connect('discord.db') as conn:
            ...
            # Получаем актуальные данные участника (включая coins_given)
            c.execute("SELECT * FROM users WHERE user_id = ?", (member.id,))
            user_data = c.fetchone()

            if user_data:
                # Обновляем распаковку, чтобы учитывать все столбцы в таблице
                user_id, username, avatar, messages_sent, reactions_sent, time_spent_in_voice_channels, coins, birthday, exp, level, roles, coins_given, presence = user_data

                # Логирование для отладки
                print(f"Данные участника: {user_data}")

                # Сохраняем все данные при выходе, включая coins_given
                c.execute(
                    """UPDATE users 
                    SET 
                        username = ?, avatar = ?, messages_sent = ?, reactions_sent = ?, 
                        time_spent_in_voice_channels = ?, coins = ?, birthday = ?, 
                        exp = ?, level = ?, roles = ?, coins_given = ?, presence = 0 
                    WHERE user_id = ?""",
                    (username, avatar, messages_sent, reactions_sent,
                     time_spent_in_voice_channels, coins, birthday,
                     exp, level, roles_str, coins_given, member.id)
                )
                conn.commit()
    ...
    try:
        ...
        embed = disnake.Embed(title=f"Участник {member.name} покинул сервер", color=disnake.Color.red())
...
async def card_user(ctx, member: disnake.Member = None):
    ...
    with sqlite3.connect('discord.db') as conn:
        ...
        # Логика для определения топа по накопленному опыту, учитывая только участников с presence = 1
        c.execute("SELECT COUNT(*) FROM users WHERE exp > ? AND presence = 1", (experience,))
...
async def temp_role(ctx, member: disnake.Member, role: disnake.Role, duration: str):
    """
    Выдаёт роль пользователю на определённое количество времени и записывает информацию в БД.
    ...
    with sqlite3.connect('discord.db') as conn:
        c = conn.cursor()
        c.execute("""
            INSERT OR REPLACE INTO user_roles (user_id, role_id, expire_date)
            VALUES (?, ?, ?)
        """, (member.id, role.id, end_time.isoformat()))
        conn.commit()
...
def generate_embed(page):
    ...
    price_category_name = \
        ["ЦЕНА НА МЕСЯЦ", "ЦЕНА НА 3 МЕСЯЦА", "ЦЕНА НА ПОЛГОДА", "ЦЕНА НА ГОД", "ЦЕНА НАВСЕГДА"][page]
...
@bot.command()
@commands.has_any_role(1135313711695921222, 1123262857614721104)  # Проверка на роли
...
async def unmute_all(ctx):
    ...
    # Получаем всех участников голосового канала
    members = voice_channel.members
    if not members:
        await ctx.send("В голосовом канале никого нет.")
        return

    # Размут всех участников
    try:
        for member in members:
            if member.voice and member.voice.mute:  # Проверяем, находится ли пользователь в мьюте
                await member.edit(mute=False)

        await ctx.send(f"Все участники в {voice_channel.mention} размучены.")
    except disnake.Forbidden:
        await ctx.send("У меня недостаточно прав для выполнения этой операции.")
    except Exception as e:
...
@bot.command(name="rename_voice", aliases=["переименовать"])
@commands.has_permissions(administrator=True)
async def rename_voice(ctx):
    """Переименовать всех в голосовом канале на номера."""
    if ctx.author.voice and ctx.author.voice.channel:
        voice_channel = ctx.author.voice.channel
        members = [member for member in voice_channel.members if member != ctx.author]

        if not members:
            await ctx.send("В голосовом канале нет других участников.")
            return

        for i, member in enumerate(members, start=1):
            try:
                await member.edit(nick=str(i))
            except Exception as e:
                await ctx.send(f"Не удалось переименовать {member.display_name}: {e}")

        await ctx.send(f"Участники голосового канала {voice_channel.name} переименованы.")
    else:
        await ctx.send("Вы не находитесь в голосовом канале.")


@bot.command(aliases=['синхронизация', 'sync_presence'])
@commands.has_permissions(administrator=True)
async def sync_server_presence(ctx):
    """
    Синхронизирует данные участников сервера с базой данных, устанавливая presence = 1 для присутствующих.
    """
    try:
        ...
        with sqlite3.connect('discord.db') as conn:
            ...
            # Сначала сбрасываем presence у всех пользователей
            c.execute("UPDATE users SET presence = 0")

            # Получаем список всех участников на сервере
            members = ctx.guild.members

            # Обновляем presence для участников, которые есть на сервере
            for member in members:
        ...
        await ctx.send(f"Синхронизация завершена. Присутствие обновлено для {len(members)} участников.")

    except Exception as e:
        await ctx.send(f"Произошла ошибка при синхронизации: {e}")
        print(f"Ошибка при синхронизации: {e}")

@bot.command()
@commands.has_permissions(administrator=True)
async def give_all_coins(ctx, amount: int):
    """
    Выдаёт указанное количество монет всем участникам сервера.
    """
    if amount <= 0:
        await ctx.send("Количество монет должно быть больше нуля.")
        return
    ...
    try:
        with sqlite3.connect('discord.db') as conn:
            ...
            # Выдаём монеты каждому участнику
            for member in members:
                if not member.bot:  # Пропускаем ботов
                    ...
                    if result:
                        # Обновляем монеты, если участник уже есть в БД
                        current_coins = result[0]
                        new_coins = current_coins + amount
                        c.execute("UPDATE users SET coins = ? WHERE user_id = ?", (new_coins, member.id))
                    else:
                        # Добавляем участника в БД, если его там нет
                        c.execute("INSERT INTO users (user_id, username, coins) VALUES (?, ?, ?)",
                                  (member.id, member.name, amount))

            conn.commit()

        await ctx.send(f"Успешно выдано {amount} монет всем участникам сервера!")
    except Exception as e:
        print(f"Ошибка: {e}")
        await ctx.send("Произошла ошибка при выдаче монет.")

@bot.command()
async def start_mafia(ctx, *, roles_input: str):
    """
    Команда для переименования участников голосового канала и раздачи ролей.
    Пример использования: !start_mafia Мафия 2, Мирный 6, Доктор 1, Комиссар 1
    """
    # Проверяем, есть ли у пользователя разрешённые роли
    allowed_roles = {1135313711695921222, 1123262857614721104}
    ...
    # Проверяем, достаточно ли участников для раздачи ролей
    roles_data = {}
    total_roles = 0
    try:
        for role_input in roles_input.split(","):
            role_name, role_count = role_input.strip().rsplit(" ", 1)
            role_count = int(role_count)
            roles_data[role_name] = role_count
            total_roles += role_count
    except ValueError:
        await ctx.send("Неверный формат ввода. Используйте формат: Роль Количество, Роль Количество (например, Мафия 2, Мирный 6).")
        return

    if total_roles != len(members):
        await ctx.send(f"Количество участников ({len(members)}) не совпадает с общим количеством ролей ({total_roles}).")
        return

    # Перемешиваем участников и роли
    random.shuffle(members)
    roles = [role for role, count in roles_data.items() for _ in range(count)]
    random.shuffle(roles)

    # Переименовываем участников и раздаем роли
    for i, member in enumerate(members, start=1):
        new_name = f"{i}"
        try:
            await member.edit(nick=new_name)
        except disnake.Forbidden:
            await ctx.send(f"Не удалось переименовать {member.display_name} из-за отсутствия прав.")
            continue

        role_assigned = roles.pop()
        try:
            await member.send(f"Ваша роль: {role_assigned}")
        except disnake.Forbidden:
            await ctx.send(f"Не удалось отправить сообщение {member.display_name} из-за закрытых личных сообщений.")

    await ctx.send("Роли успешно распределены и участники переименованы!")
