...
def setup(bot):
    ...
    async def on_ready():
        ...
        try:
            ...
            # Вызов функции для проверки истекших ролей
            bot.loop.create_task(run_check_role_expiry())  # Запуск задачи на регулярную проверку ролей
            bot.loop.create_task(run_check_birthdays())
    ...
    async def on_member_join(member):
        try:
            with sqlite3.connect('discord.db') as conn:
                ...
                if result:
                    user_id, username, avatar, messages_sent, reactions_sent, time_spent_in_voice_channels, coins, birthday, exp, level, roles, coins_given, presence = result
                    ...
                    # Обновление присутствия участника
                    c.execute("UPDATE users SET presence = 1 WHERE user_id = ?", (member.id,))
                else:
                    # Если участника нет, добавляем его в базу данных с полем presence = 1
                    c.execute("""
                        INSERT INTO users (user_id, username, presence)
                        VALUES (?, ?, 1)
    ...
    role_multipliers = {
        ...
        "1123262857614721104": 2,  # Роль 2: 2x монеты
        "760998034792251409": 2, # Роль 2: 2x монеты
        "1321989501824012390": 2, # Роль 2: 2x монеты
        "1321989493179547732": 3, # Роль 2: 3x монеты
        "1321989484291686441": 5, # Роль 2: 5x монеты
        "760998034845728786": 3,  # Роль 2: 3x монеты
    ...
    async def check_level_up(member):
        with sqlite3.connect('discord.db') as conn:
            ...
            if result:
                exp, level, coins = result  # Извлекаем опыт и текущий уровень
                new_level = level  # Начинаем с текущего уровня

                # Список для хранения сообщений о повышении уровня
                level_up_messages = []

                while True:
                    # Расчет опыта, необходимого для следующего уровня
                    if new_level == 0:
                        exp_to_next_level = 100  # Для перехода с 0 на 1 уровень
                    else:
                        exp_to_next_level = 100 * ((new_level+1) ** 2)  # Для всех последующих уровней

                    # Проверяем, хватает ли опыта для повышения уровня
                    if exp >= exp_to_next_level:
                        new_level += 1
                        level_up_messages.append(
                            f"Вы достигли уровня {new_level}")  # Добавляем сообщение о повышении уровня
                    else:
                        break

                # Обновляем уровень и монеты только если есть повышения
                if level < new_level:
                    # Обновляем уровень и добавляем 1000 монет за каждый новый уровень
                    c.execute("UPDATE users SET level = ?, coins = coins + 100 * ? WHERE user_id = ?",
                    ...
                    for level_up in level_up_messages:
                        await member.send(f"Поздравляем! Ты достиг {level_up} уровня и получил 1000 {emoji}!")
    ...
    async def generate_level_image(user: disnake.User, level: int, exp: int, exp_to_next: int):
        ...
        # Добавление текста о том, сколько опыта осталось до следующего уровня
        exp_next_level = 100 * ((level + 1) ** 2)  # Опыт, необходимый для следующего уровня
        remaining_exp = exp_next_level - exp  # Сколько еще опыта нужно для следующего уровня
        draw.text((250, text_y + 40), f"Осталось до следующего уровня: {remaining_exp}", font=font_level, fill="black")

        # Прогресс-бар
        bar_x, bar_width, bar_height = 250, 400, 30

        # Опыт для предыдущего и следующего уровней
        exp_previous_level = 100 * (level ** 2)
        exp_to_next_level = 100 * ((level + 1) ** 2)

        # Рассчитываем прогресс в пределах текущего уровня
        progress = (exp - exp_previous_level) / (exp_to_next_level - exp_previous_level)
        progress = max(0, min(1, progress))  # Ограничиваем значение между 0 и 1
    ...
    @bot.command(name="level", aliases=["левел", "уровень", "lvl"])
    async def check_level(ctx, member: disnake.Member = None):
        ...
        with sqlite3.connect('discord.db') as conn:
            ...
            if result:
                exp, level = result
                exp_to_next_level = 100 * ((level + 1) ** 2)  # Формула для опыта
    ...
    async def set_level(ctx, member: disnake.Member, level: int):
        ...
        with sqlite3.connect('discord.db') as conn:
            ...
            if result:
                ...
                # Устанавливаем опыт таким образом, чтобы он был на уровне нового уровня.
                c.execute("UPDATE users SET level = ?, exp = ? WHERE user_id = ?",
                          (level, exp_to_next_level - 1, member.id))  # Устанавливаем опыт, нужный для уровня
                conn.commit()
                ...
                if inviter:
                    try:
                        with sqlite3.connect('discord.db') as conn:
                            ...
                            c.execute("SELECT inviter_id FROM verification WHERE new_member_id = ?", (after.id,))
                            result = c.fetchone()

                            if not result:
                                # Запись пригласителя и новенького в таблицу verification
                                c.execute("INSERT INTO verification (inviter_id, new_member_id) VALUES (?, ?)",
                                          (inviter.id, after.id))
                                conn.commit()

                                # Начисление монет только если человек не был приглашён ранее
                                c.execute(
                                    "SELECT COUNT(*) FROM verification WHERE inviter_id = ? AND new_member_id = ?",
                                    (inviter.id, after.id))
                                count = c.fetchone()[0]
    ...
    async def set_birthday(ctx, date: str):
        ...
        with sqlite3.connect('discord.db') as conn:
            ...
            try:
                # Конвертируем дату в объект datetime и проверяем формат
                birthday = datetime.strptime(date, "%d-%m-%Y").date()
                formatted_birthday = birthday.strftime("%d-%m-%Y")  # Преобразуем в формат DD-MM-YYYY
            except ValueError:
                await ctx.send("Неверный формат даты. Пожалуйста, используйте формат DD-MM-YYYY.")
                return

            # Сохраняем в базе данных в формате DD-MM-YYYY
            c.execute("INSERT OR IGNORE INTO users (user_id, birthday, coins) VALUES (?, ?, ?)",
                      (user_id, formatted_birthday, 1000))
            c.execute("UPDATE users SET birthday = ? WHERE user_id = ?", (formatted_birthday, user_id))

            # Начисляем 1000 монет пользователю
            c.execute("UPDATE users SET coins = COALESCE(coins, 0) + 1000 WHERE user_id = ?", (user_id,))
            conn.commit()

            await ctx.send(f"Ваш день рождения успешно установлен на {formatted_birthday}. Вам начислено 1000 {emoji}!")

    async def check_birthdays():
        print("Running 'check_birthdays' task.")
        try:
            guild = bot.get_guild(760998034821349436)  # ID сервера
            if not guild:
                print("Error: Guild not found. Check the server ID.")
                return

            general_channel = guild.get_channel(760998035483262976)  # Общий канал для поздравлений
            log_channel = guild.get_channel(760998035483262977)  # Канал для логирования
            birthday_role = guild.get_role(760998034845728786)  # Роль для именинников

            # Получаем текущую дату с учётом часового пояса Киева
            ...
            with sqlite3.connect('discord.db') as conn:
                ...
                # Получаем пользователей с датой рождения сегодня
                ...
                for user_id, birthday in birthday_users:
                    try:
                        ...
                        if birthday_date.day == current_date.day and birthday_date.month == current_date.month:
                            # Проверяем, поздравлялся ли пользователь сегодня
                            c.execute(
                                "SELECT last_congratulation_date FROM birthday_log WHERE user_id = ?",
                                (user_id,)
                            )
                            result = c.fetchone()

                            if result and datetime.strptime(result[0], "%Y-%m-%d").date() == current_date:
                                # Пользователь уже был поздравлен
                                continue

                            # Пользователь ещё не поздравлялся, выполняем поздравление
                            ...
                            if general_channel:
                                await general_channel.send(
                                    f"🎉 Поздравляем {member.mention} с днём рождения! 🥳🎂"
                                )

                            # Добавляем роль именинника
                            if birthday_role and birthday_role not in member.roles:
                                await member.add_roles(birthday_role)

                                # Логирование добавления роли
                                if log_channel:
                                    await log_channel.send(
                                        f"🎉 Роль **{birthday_role.name}** была выдана {member.mention} на 24 часа в честь дня рождения!"
                                    )

                                # Запись данных о временной роли в таблицу user_roles
                                expire_date = datetime.now() + timedelta(seconds=86400)
                                c.execute(
                                    "INSERT OR REPLACE INTO user_roles (user_id, role_id, expire_date) VALUES (?, ?, ?)",
                                    (user_id, birthday_role.id, expire_date)
                                )

                            # Начисляем монеты
                            c.execute("UPDATE users SET coins = coins + 10000 WHERE user_id = ?", (user_id,))

                            # Логирование начисления монет
                            if log_channel:
                                await log_channel.send(
                                    f"💰 {member.mention} получил 10,000 {emoji} в честь дня рождения! 🎁"
                                )

                            # Обновляем дату последнего поздравления
                            c.execute(
                                "INSERT OR REPLACE INTO birthday_log (user_id, last_congratulation_date) VALUES (?, ?)",
    ...
    async def run_check_birthdays():
        while True:
            ...
            await asyncio.sleep(3600)  #Задержка на 1 час (3600 секунд)

    # Команда для ручного запуска проверки
    @bot.command()
    async def runbirthday(ctx):
        """Запускает проверку дней рождения вручную."""
        await check_birthdays()
        await ctx.send("Birthday check has been run manually.")
    ...
    async def birthdays(ctx):
        ...
        for user_id, birthday in birthday_data:
            try:
                birthday_date = datetime.strptime(birthday, "%d-%m-%Y").date()  # Изменён формат на полный год
                days_left = (birthday_date.replace(year=current_date.year) - current_date).days

                if days_left < 0:  # Если день рождения уже прошёл в этом году
                ...
                print(f"Error parsing birthday for user {user_id}: {e}")
                continue

        # Сортировка и выбор ближайших 5 дней рождений
    ...
    async def check_role_expiry():
        ...
        try:
            guild = bot.get_guild(760998034821349436)  # Заменить на свой ID сервера
            if not guild:
                print("Error: Guild not found. Check the server ID.")
                return

            log_channel = guild.get_channel(1297662275196424334)  # Канал для логирования

            # Получаем текущую дату и время с учётом часового пояса Киева
            kiev_tz = pytz.timezone('Europe/Kiev')
            ...
            with sqlite3.connect('discord.db') as conn:
                ...
                # Получаем пользователей, у которых срок действия роли истёк
                ...
                for user_id, role_id in expired_roles:
                    # Находим участника и роль на сервере
                    member = guild.get_member(user_id)
                    ...
                    if member and role and role in member.roles:
                        ...
                        # Удаляем информацию о роли из базы данных
                        c.execute("DELETE FROM user_roles WHERE user_id = ? AND role_id = ?", (user_id, role_id))

                        # Логируем в канал
                        if log_channel:
                            await log_channel.send(
                                f"❌ Роли были удалены **{role.name}** у **{member.display_name}**."
                            )

                        # Отправляем пользователю сообщение
                        try:
                            await member.send(f"Срок действия вашей роли '{role.name}' истек, поэтому она была удалена.")
                        except Exception as e:
            ...
            # Логируем окончание проверки
            if log_channel:
                await log_channel.send("✅ Проверка на сроки ролей успешна.")

        except Exception as e:
            print(f"Error in 'check_role_expiry': {e}")

    # Функция для асинхронного выполнения задачи с задержкой
    async def run_check_role_expiry():
        while True:
    ...
    async def runcheck(ctx):
        """Запускает проверку ролей вручную."""
        await check_role_expiry()
        await ctx.send("Role expiry check has been run manually.")
